...
class OrderController extends Controller
{
    public function place_order(Request $request)
    {
        ...
        $address = [
            'contact_person_name' => $request->contact_person_name?$request->contact_person_name:$request->user()->name.' '.$request->user()->name,
        ...
        if (!isset($request['cart']) || !is_array($request['cart']) || empty($request['cart'])) {
            return response()->json([
                'errors' => [
                    ['code' => 'cart', 'message' => 'Cart is empty or invalid']
                ]
            ], 401);
        }
        
        $product_price = 0; // Initialize the product price
        $order_details = []; // Initialize the order details array
        ...
        foreach ($request['cart'] as $c) {
            ...
            if ($product) {
                ...
                return response()->json([
                    'errors' => [
                        ['code' => 'food', 'message' => 'Food not found!']
                    ]
                ], 404); // Return a 404 response for not found
            }
        }
